...
const NAV_ADMIN = [
  ...
  { to: '/admin/payments', label: 'Payments', d: 'M3 10h18M7 15h1m4 0h1M5 6h14a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2z' },
...
function Tip({ label }) {
  return (
    <span className="absolute left-full ml-3 px-3 py-1.5 rounded-xl text-xs font-semibold
      ...
      style={{
        background: '#1e293b', color: '#e2e8f0', boxShadow: '0 4px 16px rgba(0,0,0,.4)',
...
export default function Sidebar({ collapsed, setCollapsed }) {
  ...
  const shouldAutoCollapse = window.innerWidth < 768;
  ...
  return (
    <aside style={{
      width: collapsed || shouldAutoCollapse ? 'var(--sidebar-w-sm)' : 'var(--sidebar-w)',
        ...
        <div className="relative group">
          <button onClick={() => setCollapsed(!collapsed)}
            ...
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              ...
              {shouldAutoCollapse && <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />}
            </svg>
          </button>
        </div>
      </div>
      ...
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        {items.map(({ to, label, d }) => {
          ...
          const showText = !collapsed && !shouldAutoCollapse;
          return (
            <div key={to} className="relative group">
              <Link to={to} className={`sbi ${active ? 'active' : ''}`}
                style={{ justifyContent: showText ? 'flex-start' : 'center', padding: showText ? '0.625rem 0.75rem' : '0.625rem' }}>
                <Ico d={d} />
      ...
      <div className="px-2 py-3" style={{ borderTop: '1px solid rgba(255,255,255,.06)' }}>
        <div className="relative group">
          <button onClick={() => { logout(); navigate('/login'); }}
            ...
            style={{ justifyContent: (collapsed || shouldAutoCollapse) ? 'center' : 'flex-start', padding: (collapsed || shouldAutoCollapse) ? '0.625rem' : '0.625rem 0.75rem' }}>
            <Ico d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
            {!collapsed && !shouldAutoCollapse && <span className="truncate">Logout</span>}
          </button>
          {(collapsed || shouldAutoCollapse) && <Tip label="Logout" />}
        </div>
      </div>
    </aside>
  );
